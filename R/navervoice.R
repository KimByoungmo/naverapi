...
naverVoice <- function(text = '', client_Id = '', client_secret = '', speaker = c('mijin','jinho','clara','matt','yuri','shinji','meimei'), speed = 0) {
  ...
  q <- list (speaker = s, speed = speed, text = enc2utf8(text))
  result <- POST(url = url, body = q, config = add_headers(h), encode = 'form')
